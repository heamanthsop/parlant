...
class PicoAgentStore(AgentStore):
    ...
    async def upsert_tag(
        self, agent_id: AgentId, tag_id: TagId, creation_utc: datetime | None = None
    ) -> bool:
        raise NotImplementedError
...
class ParlantServer:
    ...
    class Agent:
        ...
        async def attach_tool(
            ...
            await self._container[GuidelineToolAssociationStore].create_association(
                ...
                tool_id=ToolId(
                    service_name=_INTEGRATED_TOOL_SERVICE_NAME, tool_name=tool.tool.name
                ),
            )
    ...
    async def __aenter__(self) -> ParlantServer:
    ...
    async def create_agent(self, name: str, description: str) -> ParlantServer.Agent:
        agent = await self._container[AgentStore].create_agent(name, description)

        return ParlantServer.Agent(
            id=agent.id,
            name=agent.name,
