...
async def test_that_listing_guidelines_includes_disabled_guidelines(
    async_client: httpx.AsyncClient,
    container: Container,
    agent_id: AgentId,
) -> None:
    ...
    enabled_guideline = await guideline_store.create_guideline(
        guideline_set=agent_id,
        condition="the customer greets you",
        action="reply with 'Hello'",
    )

    disabled_guideline = await guideline_store.create_guideline(
        guideline_set=agent_id,
        condition="the customer greets you",
    ...
    await async_client.patch(
        f"/agents/{agent_id}/guidelines/{disabled_guideline.id}",
        json={"enabled": False},
    )

    response = (await async_client.get(f"/agents/{agent_id}/guidelines")).raise_for_status()

    guidelines = response.json()
    assert len(guidelines) == 1
    assert guidelines[0]["enabled"]
    ...
    assert any(g["id"] == disabled_guideline.id and not g["enabled"] for g in guidelines)
    assert any(g["id"] == enabled_guideline.id and g["enabled"] for g in guidelines)
